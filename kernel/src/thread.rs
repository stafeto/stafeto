...
//! Threads (spec 4, 8.1): a program's registers, its scheduling parameters
//! and its process, in objects from a kernel pool. The registers come
...
//! kernel stack holds nothing of it.

use crate::arch::user::{self, UserRegs};
...
/// Runs `next` at EL0: switches TTBR0 to its process's address space
/// unless TTBR0 holds it already, then returns to EL0 with its registers.
/// Never returns: the kernel stack starts over at the next entry, and no
/// value on the caller's stack is ever dropped, so the caller holds none
/// with a `Drop`: no lock guard, `AddressSpace` or the like.
pub fn run(next: NonNull<Thread>) -> ! {
    // SAFETY: `next` is alive, and so is its process; the kernel touches
    // them one at a time.
    unsafe {
