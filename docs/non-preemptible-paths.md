...
1000 times each, less the empty measurement; chunks and batches as the
longest time `KERNEL_STATS` records (`x5`, `x8`). The counts see no
...
here is without the poison, as in the normal build.
...
| `timer_set` | on a closed channel returns `PEER_CLOSED` and changes nothing; otherwise takes an armed timer off the heap, then posts bit 0 at once for a deadline the counter reached, as `notify` does, or puts the timer into the heap | O(log n): a removal and an insertion, or a removal and a post of O(1) | 916 with 64 timers in the heap |
| `timer_cancel` | takes an armed timer off the heap; bits it posted stay in its slot | O(log n) | 1,248 with a `timer_set` before it |
