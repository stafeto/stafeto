...
opt-level 2 (spec 14), measured at the end of parts 1.3c and 1.3d, and at
the end of stage 1.3 (part 1.3e) for the rows the kernel tests print, the
calls of part 1.3e and the Buffers chunk of B, in QEMU
...
request there, `ipc round trip ticks: null=307 switch=642 fast=2314
slow=2512 buffer=3396 handles=4713`: an empty call, one switch between
...
empty call, is 1591 there; the fast path saves 198, and the two copies of
960 bytes cost 884. The chunks of the stages that release handles are
...
create=19116 map=5653 map_exec=8048 unmap=2765 protect=2908
protect_exec=6343 release=16181 first_map=13869`. On the `cortex-a53`,
whose instruction cache the kernel flushes whole, `map_exec` is 5,865,
`protect_exec` 4,195 and `first_map` 11,811. Allocations are measured
...
set=2239`: the timers' part of an interrupt that finds the tops of all 63
...
`interrupt path ticks: driver=657 bind=853 ack=305 portion=250`: the
...
`device window ticks: create=899 map=7419 release=235`: the first entry
...
| `timer_set` | on a closed channel returns `PEER_CLOSED` and changes nothing; otherwise takes an armed timer off the heap of its level, then posts bit 0 at once for a deadline the counter reached, as `notify` does, or puts the timer into that heap; the nearest deadline is walked again when the top of the level changed | O(log n): a removal and an insertion, or a removal and a post of O(1); and a walk of 63 levels | 2,239 (test build): the top of a heap of 4,096 moved before every other timer, among 63 levels with timers |
| `timer_cancel` | takes an armed timer off the heap of its level, and walks the levels when its top changed; bits it posted stay in its slot | O(log n) and a walk of 63 levels | 1,261 (normal build) with a `timer_set` before it, from EL0 in a loop of 1000 with one timer armed |
...
| B, the blocking time of any thread, level 63 included (spec 15.3) | the longest row above, which a pending interrupt waits for; firings of timers are chunks of their levels, and no series of them blocks a higher level | the longest row | 20,047 under -icount (test build) at the end of stage 1.3, a chunk of the Buffers stage whose 11 frames each merge up to the highest order and whose 42 handles each wake a receiver, the longest of the rows measured, where printing costs nothing; the same chunk was 19,708 at the end of part 1.3d: part 1.3e adds 339 to it, about 8 instructions a wake of a receiver: 87 with the levels of timers, 126 with interrupt bindings among the sources of a channel, and 126 with the queue's count; B does not grow with the number of timers, bindings, slots or threads, only the cost of a unit grew, for the reasons named; then 19,354, a Buffers chunk of 32 frames, 19,318, a Handles chunk of the same kind, and 19,300, the first entry of `mem_create` with only blocks of the highest order free (part 1.3d); on hardware `debug_write` and the fault line are longer (their rows); a chunk of firings, 12,793, and every path of part 1.3e stay below it |
