...
whole pages, which touches no page of RAM, of the GIC or of the kernel's
PL011; the list of those ranges comes from the device tree at boot, 11
regions at most. A window owns no frame: its pages come from its base
...
opt-level 2 (spec 14), measured after stage 1.3, in QEMU
...
request in the test build, `ipc round trip ticks: null=250 switch=454
fast=1900 slow=2099 buffer=2999 handles=4301`: an empty call, one switch between
...
empty call, is 1158 there; the fast path saves 199, and the two copies of
...
create=11170 map=5570 map_exec=8060 unmap=2705 protect=2846
protect_exec=6286 release=16181 first_map=13824`. On the `cortex-a53`,
whose instruction cache the kernel flushes whole, `map_exec` is 5,877,
`protect_exec` 4,138 and `first_map` 11,766. Allocations are measured
...
cases there (spec 15.3), `timer portions ticks: interrupt=4388 fire=12793
set=2056`: the timers' part of an interrupt that finds the tops of all 63
...
`interrupt path ticks: driver=658 bind=688 ack=122 portion=250`: the
...
`device window ticks: create=734 map=7279 release=235`: the first entry
...
| an SError at EL0 (`exceptions::system_error`) | counts the device windows among the running process's mappings, prints them and the program's registers, and stops the machine | up to 64 mappings, then the report; the machine does not go on |  |
...
| `clock_now` | reads the counter and turns ticks into nanoseconds, rounded down | constant: one 128-bit multiplication and division | 334 |
...
| `timer_set` | on a closed channel returns `PEER_CLOSED` and changes nothing; otherwise takes an armed timer off the heap of its level, then posts bit 0 at once for a deadline the counter reached, as `notify` does, or puts the timer into that heap; the nearest deadline is walked again when the top of the level changed | O(log n): a removal and an insertion, or a removal and a post of O(1); and a walk of 63 levels | 2,056 (test build): the top of a heap of 4,096 moved before every other timer, among 63 levels with timers |
| `timer_cancel` | takes an armed timer off the heap of its level, and walks the levels when its top changed; bits it posted stay in its slot | O(log n) and a walk of 63 levels | 2,666 (test build): the top of a heap of 4,096 among 63 levels with timers |
...
| a teardown that a thread at a high level starts (`process_kill`, the last handle to a big process or to a channel with many waiters) | runs at the level of its cause (spec 7.7), the Close and Replies stages at the higher of the cause and their top waiter: the chunks of the whole teardown follow one another at that level, with interrupt polls between them, and no thread at or below that level runs until they end | each chunk as in its row; in all, the sum of the object's chunks | 42,961: `process_kill` of a child with 64 threads that never ran, with its whole teardown; closing a channel with 60 waiting receivers is no single path: two chunks of the Close stage at their level, the longer 3,844 (test build), then each of the 60 receivers it wakes runs above the closer and exits on its own, about 1,100 instructions each |
| B, the blocking time of any thread, level 63 included (spec 15.3) | the longest row above, which a pending interrupt waits for; firings of timers are chunks of their levels, and no series of them blocks a higher level | the longest row | 20,069 under -icount (test build) after stage 1.3, a chunk of the Buffers stage whose 11 frames each merge up to the highest order and whose 42 handles each wake a receiver, the longest of the rows measured, where printing costs nothing; B does not grow with the number of timers, bindings, slots or threads; next come 19,960, a Handles chunk of 64 last copies of sessions, within 0.6 % of B, and 19,354, a Buffers chunk of 32 frames; on hardware `debug_write` and the fault line are longer (their rows); a chunk of firings, 12,793, stays below it |
