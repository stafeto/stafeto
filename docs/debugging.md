...
4. Сбой программы на EL0 ядро пока тоже останавливает (завершать только
   процесс оно начнёт на этапе 1.2в). Отчёт начинается строкой
   `program fault at EL0, thread 0x...`, за ней идут регистры программы
   (x0..x30, `sp_el0`, `elr`, `spsr`, `tpidr_el0`), вид ошибки доступа и
   паника с именем входа `EL0 sync`. Адреса в `elr` и `FAR` здесь адреса
   программы; стек вызовов показывает только функции ядра, которые приняли
   исключение. SError или FIQ, принятые на EL0, дают такой же отчёт со
   строкой `system error at EL0, thread 0x...`: это ошибка системы
   (например, внешняя ошибка записи ядра в устройство), программа к ней
   непричастна.
...
`target/stafeto-ktest.elf` для сборки с тестами ядра, `target/stafeto-probe.elf`
и `target/stafeto-overflow.elf` для пробников сбоя (неизвестная инструкция и
переполнение стека) из `cargo xtask test`. Нужный файл называет сама строка
