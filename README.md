...
Подпроект 1, этап 1.2а (память ядра) готов. Ядро в формате arm64 Image
загружается в QEMU, читает дерево устройств и работает в верхней половине
адресов на собственных таблицах страниц: W^X, сторожевая страница под стеком,
вся RAM в прямом отображении. Физическую память оно раздаёт методом близнецов,
объекты ядра берёт из пулов, дескрипторы хранит в таблице с поколениями и
правами. При сбое ядро печатает регистры, класс исключения и стек вызовов.
Следующий этап 1.2б: прерывания, таймер, потоки, пользовательский режим и
планировщик.
